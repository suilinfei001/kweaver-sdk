...
def test_datasource_test_connectivity(adp_client: ADPClient, db_config: dict[str, Any]):
    """Verify that we can reach the configured test database."""
    result = adp_client.datasources.test(**db_config)
    assert result is True


def test_datasource_create_and_list(create_datasource, adp_client: ADPClient):
    """Create a datasource and verify it appears in list."""
    ds = create_datasource(name="e2e_ds_list_test")

    found = adp_client.datasources.list(keyword="e2e_ds_list_test")
    assert any(d.id == ds.id for d in found)


def test_datasource_get(create_datasource, adp_client: ADPClient):
    """Create a datasource and retrieve it by ID."""
    ds = create_datasource(name="e2e_ds_get_test")
    fetched = adp_client.datasources.get(ds.id)
    assert fetched.id == ds.id
    assert fetched.name == "e2e_ds_get_test"


def test_datasource_list_tables(create_datasource, adp_client: ADPClient):
